...
public Action CommandTop(int client, int args) {
	...
	g_PlayerTopStyle[client] = view_as<KZStyle>(SimpleKZ_GetOption(client, KZOption_Style));
...
public Action CommandMapTop(int client, int args) {
	if (args == 0) {
		...
		DB_OpenMapTop(client, SimpleKZ_GetCurrentMapID(), 0, view_as<KZStyle>(SimpleKZ_GetOption(client, KZOption_Style)));
	}
	else if (args >= 1) {
		// Open map top for specified map and their current style
		char specifiedMap[33];
		GetCmdArg(1, specifiedMap, sizeof(specifiedMap));
		DB_OpenMapTop_FindMap(client, specifiedMap, 0, view_as<KZStyle>(SimpleKZ_GetOption(client, KZOption_Style)));
	}
	return Plugin_Handled;
}
...
public Action CommandBMapTop(int client, int args) {
	if (args == 0) {
		...
		DB_OpenMapTop(client, SimpleKZ_GetCurrentMapID(), 1, view_as<KZStyle>(SimpleKZ_GetOption(client, KZOption_Style)));
	}
	else if (args == 1) {
		...
		int bonus = StringToInt(argBonus);
		if (bonus > 0) {
			DB_OpenMapTop_FindMap(client, argMap, bonus, view_as<KZStyle>(SimpleKZ_GetOption(client, KZOption_Style)));
		}
		else {
			CPrintToChat(client, "%t %t", "KZ Prefix", "Invalid Bonus Number", argBonus);
		}
...
public Action CommandPB(int client, int args) {
	if (args == 0) {
		...
		DB_PrintPBs(client, SimpleKZ_GetPlayerID(client), SimpleKZ_GetCurrentMapID(), 0, view_as<KZStyle>(SimpleKZ_GetOption(client, KZOption_Style)));
	}
	else if (args == 1) {
		// Print their PBs for specified map and their current style
		char argMap[33];
		GetCmdArg(1, argMap, sizeof(argMap));
		DB_PrintPBs_FindMap(client, SimpleKZ_GetPlayerID(client), argMap, 0, view_as<KZStyle>(SimpleKZ_GetOption(client, KZOption_Style)));
	}
	else if (args >= 2) {
		...
		DB_PrintPBs_FindPlayerAndMap(client, argPlayer, argMap, 0, view_as<KZStyle>(SimpleKZ_GetOption(client, KZOption_Style)));
	}
	return Plugin_Handled;
}
...
public Action CommandBPB(int client, int args) {
	if (args == 0) {
		...
		DB_PrintPBs(client, SimpleKZ_GetPlayerID(client), SimpleKZ_GetCurrentMapID(), 1, view_as<KZStyle>(SimpleKZ_GetOption(client, KZOption_Style)));
	}
	else if (args == 1) {
		...
		int bonus = StringToInt(argBonus);
		if (bonus > 0) {
			DB_PrintPBs(client, SimpleKZ_GetPlayerID(client), SimpleKZ_GetCurrentMapID(), bonus, view_as<KZStyle>(SimpleKZ_GetOption(client, KZOption_Style)));
		}
		else {
			CPrintToChat(client, "%t %t", "KZ Prefix", "Invalid Bonus Number", argBonus);
		}
		...
		int bonus = StringToInt(argBonus);
		if (bonus > 0) {
			DB_PrintPBs_FindMap(client, SimpleKZ_GetPlayerID(client), argMap, bonus, view_as<KZStyle>(SimpleKZ_GetOption(client, KZOption_Style)));
		}
		else {
			CPrintToChat(client, "%t %t", "KZ Prefix", "Invalid Bonus Number", argBonus);
		}
		...
		int bonus = StringToInt(argBonus);
		if (bonus > 0) {
			DB_PrintPBs_FindPlayerAndMap(client, argPlayer, argMap, bonus, view_as<KZStyle>(SimpleKZ_GetOption(client, KZOption_Style)));
		}
		else {
			CPrintToChat(client, "%t %t", "KZ Prefix", "Invalid Bonus Number", argBonus);
		}
...
public Action CommandWR(int client, int args) {
	if (args == 0) {
		...
		DB_PrintRecords(client, SimpleKZ_GetCurrentMapID(), 0, view_as<KZStyle>(SimpleKZ_GetOption(client, KZOption_Style)));
	}
	else if (args >= 1) {
		// Print record times for specified map and their current style
		char argMap[33];
		GetCmdArg(1, argMap, sizeof(argMap));
		DB_PrintRecords_FindMap(client, argMap, 0, view_as<KZStyle>(SimpleKZ_GetOption(client, KZOption_Style)));
	}
	return Plugin_Handled;
}
...
public Action CommandBWR(int client, int args) {
	if (args == 0) {
		...
		DB_PrintRecords(client, SimpleKZ_GetCurrentMapID(), 1, view_as<KZStyle>(SimpleKZ_GetOption(client, KZOption_Style)));
	}
	else if (args == 1) {
		...
		int bonus = StringToInt(argBonus);
		if (bonus > 0) {
			DB_PrintRecords(client, SimpleKZ_GetCurrentMapID(), bonus, view_as<KZStyle>(SimpleKZ_GetOption(client, KZOption_Style)));
		}
		else {
			CPrintToChat(client, "%t %t", "KZ Prefix", "Invalid Bonus Number", argBonus);
		}
		...
		int bonus = StringToInt(argBonus);
		if (bonus > 0) {
			DB_PrintRecords_FindMap(client, argMap, bonus, view_as<KZStyle>(SimpleKZ_GetOption(client, KZOption_Style)));
		}
		else {
			CPrintToChat(client, "%t %t", "KZ Prefix", "Invalid Bonus Number", argBonus);
		}
...
public Action CommandPC(int client, int args) {
	if (args < 1) {
		DB_GetCompletion(client, client, view_as<KZStyle>(SimpleKZ_GetOption(client, KZOption_Style)), true);
	}
	else if (args >= 1) {
		// Print record times for specified map and their current style
		char argPlayer[MAX_NAME_LENGTH];
		GetCmdArg(1, argPlayer, sizeof(argPlayer));
		DB_GetCompletion_FindPlayer(client, argPlayer, view_as<KZStyle>(SimpleKZ_GetOption(client, KZOption_Style)));
	}
	return Plugin_Handled;
}
